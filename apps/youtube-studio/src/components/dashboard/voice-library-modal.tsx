...
export function VoiceLibraryModal({
    ...
    const filteredVoices = useMemo(() => {
        ...
        return byCategory.filter((v) => v.name.toLowerCase().includes(q) || v.id.toLowerCase().includes(q));
                ...
                <div className="space-y-6">
                    ...
                    {categories.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                            ...
                            {categories.map((cat) => {
                                const count = voicesList.filter((v) =>
                                    v.name.toLowerCase().includes(cat)
                                ).length;
                                return (
                                    <Button
                                        key={cat}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {filteredVoices.map((v) => {
                            ...
                            return (
                                <div
                                    key={v.id}
                                    className={`border-2 rounded-lg p-5 space-y-4 transition-all duration-200 cursor-pointer hover:shadow-lg flex flex-col h-full ${isSelected ? 'border-primary bg-primary/5 shadow-md' : 'border-border hover:border-primary/50'
                                        }`}
                                >
                                    {/* Header with name */}
                                    <div>
                    ...
                    {filteredVoices.length === 0 && (
                        <div className="text-center py-12">
                            ...
                            <p className="text-muted-foreground">
                                No voices in this category. Try another.
                            </p>
                        </div>
                    )}
                </div>
